...
using Solitaire.Controls.ObjectListView.DragDrop;
...
#pragma warning restore 168
        ...
        public virtual void CalculateReasonableTileSize()
        {
            ...
            var tileHeight = Math.Max((int) TileSize.Height, Math.Max(imageHeight, dataHeight));
        ...
        public virtual void InsertObjects(int index, ICollection modelObjects)
        {
            ...
            try
            {
                ...
                {
                    ListViewItemSorter = null;
                    index = Math.Max(0, Math.Min(index, (int) GetItemCount()));
                    var i = index;
        ...
        internal Point LowLevelScrollPosition =>
            new Point(NativeMethods.GetScrollPosition(this, true),
        ...
        protected OlvListViewHitTestInfo LowLevelHitTest(int x, int y)
        {
            ...
            if (ShowGroups && OlvGroups != null)
            {
                ...
                else
                {
                    if (isGroupHit)
                    {
                        foreach (var olvGroup in Enumerable.Where<OlvGroup>(OlvGroups, olvGroup => olvGroup.GroupId == index))
                        {
                            group = olvGroup;
                            break;
                        }
